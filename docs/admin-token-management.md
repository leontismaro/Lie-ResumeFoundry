...
后台控制台用于管理访问 token 的完整生命周期，覆盖以下操作：
...
后台创建 token 时的“目标路径”下拉选项不是手写列表，而是构建期自动生成。
...
- session 最晚不会晚于 token 过期

## 状态与操作规则

后台会根据 token 状态显示可执行操作。

### 禁用

禁用 token 后：

- 该 token 不再允许兑换新的 session
- 已存在的 session 默认不受影响
- 可选同时吊销该 token 已签发的 session
...
- 为次数已耗尽或即将耗尽的 token 增加 `max_uses`
